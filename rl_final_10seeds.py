...
def clean_corpora(targets):
    for h, _, _ in targets:
        d = f"/tmp/corpus_final_{h}"
        if os.path.exists(d):
            shutil.rmtree(d)
        os.makedirs(d)

def run_fuzzer(harness, folder, lf_seed, runs=20):
    cmd = f"./{folder}/{harness} /tmp/corpus_final_{harness} -runs={runs} -timeout=1 -seed={lf_seed}"
    try:
...
class FuzzEnv(gym.Env):
    ...
    def step(self, action):
        ...
        new_ft = max(old_ft, run_fuzzer(harness, folder, lf_seed, runs=20))
...
def run_ppo(targets, base_seed, episodes, steps_per_episode):
    ...
    # n_steps is per env per update. We do an update every episode.
    ...
    env.episode_rewards.append(env.current_ep_reward) # append last one
    # Only return the first 'episodes' rewards, occasionally it runs 1 extra step due to timestep boundaries
    return env.episode_rewards[:episodes]
...
def run_all(targets_name, targets, steps_per_episode):
    ...
    for s in seeds:
        ...
        q_res.append(q[-1])
        ucb_res.append(ucb[-1])
        ppo_res.append(ppo[-1])
