...
def run_ucb1(base_seed, agent_multipliers, eval_multipliers, episodes=15, steps_per_episode=13):
    ...
    for ep in range(episodes):
        ...
        for step in range(steps_per_episode):
            ...
            old_ft = fts[action]
            lf_seed = base_seed + ep * 1000 + step  # Guarantee deterministic identical fuzzer state for this exact pull sequence
            new_ft = max(old_ft, run_fuzzer(TARGETS[action], lf_seed, runs=20))
            fts[action] = new_ft
