...
def run_experiment(seed, episodes=15, steps_per_episode=8):
    random.seed(seed)
    ...
    for ep in range(episodes):
        ...
        tot_r = 0
        state = get_state(fts)
        eps = max(0.05, 0.5 - (ep * 0.03)) # slightly faster decay for 15 episodes
...
def main():
    print("Starting REAL libFuzzer experiment (3 seeds, 15 episodes)...")
    seeds = [111, 222, 333]
    episodes = 15
    ...
    for pt in [5, 10, 15]:
        ...
        print(f"Episode {pt} (Average across 3 seeds):")
