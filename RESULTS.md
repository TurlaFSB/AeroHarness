...
* **Methodology Flag (Cross-Architecture Consistency)**: Initial cross-architecture comparison suggested consistent ~83% performance across Zephyr/STM32 and Kinetis. A prompt-methodology inconsistency was subsequently found between the two Stage 2 scripts and has been corrected. Re-evaluation using the corrected prompt, currently only completed via the Ollama backend (Gemini quota-limited), shows Kinetis accuracy of 52.8% (down from 56.0% under the flawed prompt) — both are Ollama-only figures and are NOT directly comparable to the Gemini-based 83.3% Zephyr result. A matched Gemini spot-check under the corrected prompt is pending (quota-limited, ~20 requests/day). This should be treated as an open question, not a resolved one, until that spot-check is complete.
